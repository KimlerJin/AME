...
public class FieldUtils {
    ...
    public static String toUnderscore(String camel) {
        ...
        if (camel != null && camel.length() > 0) {
            ...
            for (int i = 1; i < camel.length(); i++) {
                String s = camel.substring(i, i + 1);
                // 在大写字母前添加下划线
    ...
    public static String toCamel(String underscore) {
        ...
        if (underscore == null || underscore.isEmpty()) {
            // 没必要转换
            return "";
        } else if (!underscore.contains("_")) {
            // 不含下划线，仅将首字母小写
            return underscore.toLowerCase();
        }
        // 用下划线将原始字符串分割
        String camels[] = underscore.split("_");
    ...
    // System.out.println(toUnderscore("desc60"));
    // }
}
